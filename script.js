...
function renderQuestion() {
    ...
    document.getElementById("options").innerHTML = data.opts
        .map((opt, i) =>
            `<div class="opt-row" data-key="${keys[i]}">
            <span class="opt-check-custom"></span>
            <span class="opt-letter">${keys[i]})</span>
            <span class="opt-text-label">${opt}</span>
        </div>`
        ).join("");
...
function pick(el, key) {
    ...
    if (el.classList.contains("selected")) {
        ...
        chosenKeys = chosenKeys.filter(k => k !== key);
    }
...
function nextQ() {
    ...
    chosenKeys.forEach(key => scores[key]++);
    current++;
